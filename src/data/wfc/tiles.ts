...
const BASE_MATRIX: number[][] = (() => {
  ...
  // SURFACE_* ↔ SOLID: 0.3
  m[S.SURFACE_LOW][S.SOLID] = 0.3;
  m[S.SURFACE_HIGH][S.SOLID] = 0.3;
  m[S.DOUBLE_SURFACE][S.SOLID] = 0.3;
  m[S.SOLID][S.SURFACE_LOW] = 0.3;
  m[S.SOLID][S.SURFACE_HIGH] = 0.3;
  m[S.SOLID][S.DOUBLE_SURFACE] = 0.3;
...
function expandWithMirrors(baseTiles: WfcTile[]): WfcTile[] {
  ...
  for (const tile of result) {
    ...
    for (const dir of Object.keys(tile.mandatoryNeighbors) as Direction[]) {
      tile.mandatoryNeighbors[dir] = tile.mandatoryNeighbors[dir]!
        .map((id) => {
